...
def summarize_smt_transactions():
    ...
    try:
        ...
        try:
            ...
            type_idx = col_map["계좌 분류"] # '자산 분류' 대신 '계좌 분류' 사용
            qty_idx = col_map["주식수 (매도: 마이너스)"]
            acc_idx = col_map["계좌번호"]
        except KeyError as e:
            print(f"컬럼 매핑 오류: {e}")
            print(f"확인된 컬럼 목록: {headers}")
            return
        ...
        if summary:
            for acc in sorted(summary.keys()):
                print(f" 계좌번호: {acc:<18} | 합산수량: {summary[acc]:>12,.0f}")
        else:
            print(" 해당 조건(SMT & 2025.12.31 이전)에 맞는 데이터가 없습니다.")
        print("="*50)
